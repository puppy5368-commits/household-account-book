...
const DEFAULT_RECURRING = [
  ...
  {item_type:"expense",name:"희윤 용돈",amount:30000,category:"자녀용돈/적립",person_name:"민정",account_name:"민정 7777",due_day:null,is_variable:false,memo:"9월 지급완료 · 현금 15,000 + 계좌 15,000"},
...
const DEFAULT_TRANSACTIONS = [
  ...
  {tx_date:"2026-09-10",tx_type:"expense",amount:15000,description:"희윤 9월 용돈",category:"자녀용돈/적립",person_name:"민정",account_name:"민정 7777",memo:"9월 총 용돈 30,000원 지급완료 · 계좌 15,000 + 현금 15,000 · 현금은 기존 현금에서 지급"},
...
async function migrateSeptember2026(){
  ...
  const ensureRecurring=async item=>{if(!item)return;const found=(recs||[]).find(x=>x.active&&x.name===item.name);if(found)return;const {error}=await db.from("recurring_items").insert({...item,household_id:householdId,active:true});if(error)throw error;};
  ...
  const allowance=(recs||[]).find(x=>x.active&&(x.name==="희윤 용돈"||x.name==="희윤 용돈통장 · 카뱅1349"));
  if(allowance){const {error}=await db.from("recurring_items").update({name:"희윤 용돈",item_type:"expense",amount:30000,category:"자녀용돈/적립",person_name:"민정",account_name:"민정 7777",memo:"9월 지급완료 · 현금 15,000 + 계좌 15,000"}).eq("id",allowance.id);if(error)throw error;}else await ensureRecurring(DEFAULT_RECURRING.find(x=>x.name==="희윤 용돈"));
  ...
  const dupAllowance=(txs||[]).filter(x=>x.tx_type==="transfer"&&x.description==="희윤 용돈 · 계좌 지급"&&Number(x.amount)===15000);
  for(const tx of dupAllowance){const {error}=await db.from("transactions").delete().eq("id",tx.id).eq("household_id",householdId);if(error)throw error;}
  const ensureTx=async item=>{if(!item)return;if((txs||[]).some(x=>x.tx_type===item.tx_type&&Number(x.amount)===Number(item.amount)&&x.description===item.description))return;const {error}=await db.from("transactions").insert({...item,household_id:householdId});if(error)throw error;};
  for(const name of ["7777 → 2115 고정비통장","희윤 9월 용돈","가족회비"]){await ensureTx(DEFAULT_TRANSACTIONS.find(x=>x.description===name));}
}
...
function renderSummary(){
  ...
  $("#remainingFormula").textContent=`수입 ${won(income)} − 이번 달 고정지출 ${won(plannedObligations)} − 이전달 미납납부 ${won(arrears)} − 생활지출 ${won(livingSpent)}`;
...
function recurringPaidTransaction(item){
  if(item.name==="개인회생 1")return transactions.find(x=>x.tx_type==="expense"&&x.description==="개인회생 1 · 9월분");
  if(item.name==="희윤 용돈")return transactions.find(x=>x.tx_type==="expense"&&x.category==="자녀용돈/적립"&&/희윤.*용돈|용돈.*희윤/.test(x.description||""));
  return transactions.find(x=>x.tx_type==="expense"&&(x.memo||"").includes(`고정지출:${item.id}`)) || transactions.find(x=>x.tx_type==="expense"&&x.description===item.name&&!/미납분/.test(`${x.description||""} ${x.memo||""}`));
}
...
function renderRecurringGroup(listSel,totalSel,items){
  const total=items.reduce((s,x)=>s+Number(x.amount),0);$(totalSel).textContent=won(total);
  $(listSel).innerHTML=items.map(x=>{const paid=recurringPaidTransaction(x);const paidLabel=paid?`✓ 출금완료${Number(paid.amount)!==Number(x.amount)?` · ${won(paid.amount)}`:""}`:"○ 예정";return `<div class="fixed-item"><div class="fixed-item-copy"><strong>${escapeHtml(x.name)}</strong><small>${escapeHtml(x.account_name||"")} · ${x.is_variable?"변동":"고정"}${x.memo?" · "+escapeHtml(x.memo):""}</small></div><div class="fixed-item-side"><strong>${won(x.amount)}</strong><button type="button" class="fixed-pay-btn ${paid?"is-paid":""}" data-paid-recurring="${x.id}" ${paid?"disabled":""}>${paidLabel}</button></div></div>`;}).join("")||`<p class="sub">등록된 항목이 없어요.</p>`;
}

async function markRecurringPaid(id){
  const item=recurringItems.find(x=>String(x.id)===String(id)); if(!item||recurringPaidTransaction(item))return;
  let amount=Number(item.amount);
  if(item.is_variable){const value=prompt(`${item.name} 실제 출금 금액을 입력해 주세요.`,String(amount));if(value===null)return;amount=Number(String(value).replace(/[^0-9]/g,""));if(!amount||amount<1)return alert("금액을 다시 확인해 주세요.");}
  if(!confirm(`${item.name}\n${won(amount)} 출금완료로 기록할까요?`))return;
  const row={household_id:householdId,tx_date:localISODate(),tx_type:"expense",amount,description:item.name,category:item.category||"기타",person_name:item.person_name||membership.display_name,account_name:item.account_name||"카드/기타",memo:`고정지출 출금완료 · 고정지출:${item.id}`};
  const {error}=await db.from("transactions").insert(row);if(error)return alert(`저장 실패: ${error.message}`);await loadData();
}
document.addEventListener("click",e=>{const btn=e.target.closest("[data-paid-recurring]");if(btn&&!btn.disabled)markRecurringPaid(btn.dataset.paidRecurring);});
...
function iconFor(tx){if(tx.tx_type==="income")return "💰";if(tx.tx_type==="transfer")return "↔️";const map={"식비/간식":"🥨","외식/카페":"☕","생활용품":"🧻","교회/헌금":"⛪","개인회생/채무":"💳","보험":"🛡️","통신":"📱","차량":"🚙","자녀교육":"📚","자녀용돈/적립":"🪙","주거/공과금":"🏠","의료":"🏥","쇼핑/미용":"🛍️","가족회비":"👨‍👩‍👧‍👦"};return map[tx.category]||"🧾";}
